...
module GitHubStubs
  def self.valid_stubs
    ...
    # create gh-pages ref
    # create_gh_pages_ref = File.read('./spec/json/create_gh_pages_ref.json')
    # WebMock.stub_request(:post, "https://api.github.com/user/repos").to_return(status: 201, body: create_gh_pages_ref, headers: { 'Content-Type' => 'application/json' } )

    # Sha latest commit 'create ref'
    ...
    WebMock.stub_request(:patch, 'https://api.github.com/repos/test_user/path/git/refs/heads/feature').
    ...
    # Merge
    merge_request = File.read('./spec/json/merge_request.json')
    WebMock.stub_request(:post, 'https://api.github.com/repos/test_user/path/merges').to_return(status: 201, body: merge_request, headers: { 'Content-Type' => 'application/json' })
